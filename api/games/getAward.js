...
export default function(req, res, next) {
    ...
    getUserMe(token, (user, countryid) => {
        runSql(
            res,
            `SELECT id,coins,create_time FROM coins_log WHERE user_id=${user} AND TYPE=1 AND state!=2 AND fk_game=${gameId} AND create_time>'${today}' AND create_time<'${tomorrow}'`,
            result => {
                if (result.length > 0) {
                    // shang
                }
                ...
                addCoins(token, user, award, `Shot Games-${goals} goals`, addResult => {
                    ...
                    if (addResult && addResult.data.code == 0) {
                        const coinsActionid = addResult.data.id
                        const resText = JSON.stringify(addResult.data).substr(0, 800)
                        runSql(
                            res,
                            `INSERT INTO coins_log (type,coins,user_id,instruction,state,fk_game,coins_action_id,res_text,create_time) VALUES ('1',${award},${user},'Shot Games-${goals} goals',1,${gameId},${coinsActionid},${resText},'${now}')`,
                            () => {
                                res.end(
                                    JSON.stringify({
                                        code: 200,
                                        message: 'success',
                                        data: {
                                            operatorCoins: addResult.data.operatorCoins,
                                            beforeCoins: addResult.data.beforeCoins,
                                            afterCoins: addResult.data.afterCoins
                                        }
                                    })
                                )
                            }
                        ...
                        runSql(
                            res,
                            `INSERT INTO coins_log (type,coins,user_id,instruction,state,fk_game,coins_action_id,res_text,create_time) VALUES ('1',${award},${user},'Shot Games-${goals} goals',1,${gameId},'',${resText},'${now}')`,
                            () => {
                                res.end(
                                    JSON.stringify({
                                        code: 102,
